    ...
    <main className="safetyPage">
      ...
      <div className="dashboardGrid">
        ...
        <section className="panel sectionCard reportsCard"><div className="sectionHeading"><div><p className="eyebrow">Reports</p><h2>Active reports</h2></div><span className="count">{safety.activeReports.length}</span></div>
          ...
          <details className="reportComposer"><summary>Submit a new report</summary><form className="reportForm" onSubmit={handleReportSubmit}><CustomSelect name="category" label="Category" required value={reportDraft.category} placeholder="Select a category" onChange={(category) => setReportDraft((current) => ({ ...current, category }))} options={reportCategories.map(([value, label]) => ({ value, label }))} /><CustomSelect name="impact" label="Impact" required value={reportDraft.impact} placeholder="Choose impact level" onChange={(impact) => setReportDraft((current) => ({ ...current, impact }))} options={impactLevels.map(([value, label]) => ({ value, label }))} /><label className="wide">Links or thread IDs<input required value={reportDraft.link} onChange={(event) => setReportDraft((current) => ({ ...current, link: event.target.value }))} placeholder="URL, thread ID, username, or room" /></label><label className="wide">Details<textarea required rows={4} value={reportDraft.details} onChange={(event) => setReportDraft((current) => ({ ...current, details: event.target.value }))} placeholder="Describe what happened and who was involved." /></label><label className="wide">Contact (optional)<input value={reportDraft.contact} onChange={(event) => setReportDraft((current) => ({ ...current, contact: event.target.value }))} placeholder="Email or @handle" /></label><button className="primaryButton" type="submit" disabled={reportStatus.state === "loading"}>{reportStatus.state === "loading" ? "Sending…" : "Send report"}</button>{reportStatus.message ? <p className={`status ${reportStatus.state}`}>{reportStatus.message}</p> : null}</form></details>
        </section>
      ...
      <style jsx>{`
        .safetyPage{min-height:100vh;padding:20px 16px 44px;background:radial-gradient(circle at 12% 0%,rgba(0,197,255,.18),transparent 28%),linear-gradient(180deg,#06111c,#081520 55%,#060e17);color:#e4f6ff}.panel{border:1px solid rgba(73,204,255,.16);border-radius:18px;background:linear-gradient(150deg,rgba(13,34,48,.94),rgba(7,18,29,.96));box-shadow:0 14px 38px rgba(0,0,0,.24)}.hero{max-width:1180px;margin:0 auto 12px;padding:18px;display:flex;align-items:end;justify-content:space-between;gap:16px}h1,h2,p{margin:0}h1{margin-top:6px;font-size:clamp(1.7rem,4vw,2.65rem);letter-spacing:-.04em}h2{font-size:1.05rem}.hero p:last-child,.bodyCopy{margin-top:10px;color:#9db8c8;line-height:1.48}.eyebrow{color:#4bdcff;font-size:.72rem;font-weight:800;letter-spacing:.15em;text-transform:uppercase}.summaryGrid{max-width:1180px;margin:0 auto 12px;display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.summaryCard{padding:11px 13px;border:1px solid rgba(73,204,255,.13);border-radius:10px;background:rgba(8,30,44,.84);display:flex;align-items:center;justify-content:space-between;gap:10px}.summaryCard span{color:#8faebf;font-size:.76rem}.summaryCard strong{font-size:.95rem}.dashboardGrid{max-width:1180px;margin:auto;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;align-items:start}.sectionCard{padding:14px;display:grid;gap:10px}.reportsCard,.guidanceCard{grid-column:1/-1}.sectionHeading{display:flex;align-items:center;justify-content:space-between;gap:12px}.sectionHeading h2{margin-top:4px}.count{min-width:28px;padding:5px 7px;border-radius:999px;background:rgba(0,183,235,.16);color:#78e5ff;text-align:center;font-weight:800}.item{padding:10px;border:1px solid rgba(255,255,255,.07);border-radius:12px;background:rgba(255,255,255,.025)}.itemRow{display:flex;align-items:center;justify-content:space-between;gap:10px}.item>div:first-child{display:grid;gap:5px;min-width:0}.item span{color:#91adbd;font-size:.78rem;overflow-wrap:anywhere}.empty,.finePrint{color:#8fa9b8;line-height:1.55}.finePrint{font-size:.78rem}.quietButton,.primaryButton,.secondaryButton{border:0;border-radius:999px;padding:7px 11px;font:inherit;font-size:.82rem;font-weight:750;cursor:pointer;text-decoration:none;text-align:center}.quietButton{background:rgba(255,255,255,.06);color:#cdefff;white-space:nowrap}.primaryButton{background:linear-gradient(135deg,#09bce8,#35e3ff);color:#03121b}.secondaryButton{border:1px solid rgba(88,216,255,.25);background:rgba(6,77,103,.36);color:#d9f8ff}.buttonRow{display:flex;flex-wrap:wrap;gap:9px}.compactForm,.reportForm{display:grid;gap:8px}.compactForm{margin-top:9px}.reportForm{grid-template-columns:repeat(2,minmax(0,1fr));margin-top:9px}.wide{grid-column:1/-1}label{display:grid;gap:6px;color:#cce6f2;font-size:.84rem}input,textarea{box-sizing:border-box;width:100%;padding:8px 10px;border:1px solid rgba(255,255,255,.1);border-radius:10px;background:rgba(0,0,0,.19);color:#effbff;font:inherit}textarea{resize:vertical}.reportComposer,.compactComposer{border-top:1px solid rgba(255,255,255,.07);padding-top:8px}summary{color:#72e0fa;font-size:.84rem;font-weight:750;cursor:pointer}.status{padding:10px 12px;border-radius:12px;color:#b7d7e5}.status.success{background:rgba(55,216,154,.12);color:#baffdf}.status.error{background:rgba(255,100,100,.12);color:#ffc5c5}.status.info{background:rgba(39,178,225,.1)}.statusDot{width:12px;height:12px;border-radius:50%;background:#526774;box-shadow:0 0 0 5px rgba(82,103,116,.12)}.statusDot.good{background:#28e49d;box-shadow:0 0 14px rgba(40,228,157,.7)}.guidanceList{margin:0;padding-left:20px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px 18px;color:#a9c2cf;font-size:.84rem;line-height:1.4}.guidanceList strong{color:#e4f6ff}@media(max-width:800px){.summaryGrid{grid-template-columns:repeat(2,1fr)}.dashboardGrid{grid-template-columns:1fr}.reportsCard,.guidanceCard{grid-column:auto}.guidanceList{grid-template-columns:1fr}.hero{align-items:stretch;flex-direction:column}}@media(max-width:560px){.safetyPage{padding:12px 8px 36px}.reportForm{grid-template-columns:1fr}.wide{grid-column:auto}.itemRow{align-items:stretch;flex-direction:column}}
      `}</style>
    </main>
  );
}
